...
object MovieSimilarities {
  ...
  def main(args: Array[String]) {
    ...
    val base_ratings = data.map(l => l.split("\t")).map(l => (l(0).toInt, (l(1).toInt, l(2).toDouble)))
    
    // Filtering good news with ratings over 3
    val ratings = base_ratings.filter(_._2._2 > 3)
